...
begin
  ...
  if v_tables <> 30 then
    raise exception 'TEST FAILED: expected 30 tables, found %', v_tables;
  end if;
  ...
  insert into test_results values (1, 'structure: 30 tables, FKs, CHECKs', true);
end $$;
...
begin
  ...
  insert into test_results values (2, 'RLS enabled on all 30 tables', true);
end $$;
