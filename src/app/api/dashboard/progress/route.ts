...
// actual quiz attempts, never a hardcoded or guessed number (invariant 1).
//
// Join path deliberately does NOT follow the spec's literal wording
...
function bandFor(answered: number, correct: number): { band: MasteryBand; accuracy: number | null } {
...
export async function GET() {
  try {
    ...
    // 1. The universe: every chapter with real ingested textbook content in this student's
    // scope, across all their enrolled subjects — same source_type filter as /api/quiz/scope,
    // since mastery only ever makes sense for chapters a quiz could actually be generated from.
    const { data: chapterRows, error: chaptersError } = await admin
      ...
      .in('subject', enrolledSubjects)
      .eq('source_type', 'textbook');

    if (chaptersError) {
      console.error('Progress: chapters query failed:', chaptersError.message);
    ...
    if (attemptsError) {
      console.error('Progress: attempts query failed:', attemptsError.message);
      return NextResponse.json({ error: 'Failed to load progress.' }, { status: 500 });
    }
